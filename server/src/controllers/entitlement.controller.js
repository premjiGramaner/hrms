...
const createEntitlements = async (req, res, next) => {
  try {
    ...
    const year = periodStart.getMonth() >= 3
      ? periodStart.getFullYear() + 1
      : periodStart.getFullYear();

    const days = parseFloat(entitlement_days);
    if (!days || days <= 0) return error(res, 'Entitlement days must be greater than 0', 422);
    ...
    if (created_count === 0) {
      return error(res,
        `All ${skipped} entitlement(s) already exist for the selected period.`, 409
      );
    }
...
const listEntitlements = async (req, res, next) => {
  try {
    ...
    const filters = {
      employee_id:   req.query.employee_id   ? parseInt(req.query.employee_id)   : null,
    ...
    const result = await EntitlementModel.findEntitlements({ ...filters, page, limit });
...
export { getEmployees, getLeaveTypes, createEntitlements, listEntitlements, myEntitlements };
