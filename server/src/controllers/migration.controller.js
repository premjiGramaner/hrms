...
export async function uploadMigration(req, res, next) {
  try {
    ...
    return success(res, await MigrationService.uploadAndValidate(req.file, req.user), 201);
  } catch (error) {
    return next(error);
  }
}
...
export async function getMigrationErrors(req, res, next) {
  try {
    return success(res, await MigrationService.getErrors(req.params.id, req.query));
  } catch (error) {
    return next(error);
  }
}
...
export async function downloadMigrationReport(req, res, next) {
  try {
    ...
    const report = await buildMigrationReport(req.params.id, req.query.type || "all", format);
