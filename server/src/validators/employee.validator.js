...
function employeeSchema(body) {
  ...
  if (!String(value.first_name || '').trim()) return { error: 'First name is required' };
  if (!String(value.last_name || '').trim()) return { error: 'Last name is required' };
  if (!value.email) return { error: 'Work email is required' };
  if (!EMAIL_REGEX.test(value.email)) return { error: 'Enter a valid work email' };
