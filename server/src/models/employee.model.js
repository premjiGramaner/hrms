...
async function findAllEmployees(page, limit = 15) {
  ...
  const { rows: cnt } = await pool.query(
    `SELECT COUNT(*)::int as count FROM tbl_appusers WHERE is_deleted = false`
  );
...
async function updateEmployee(id, data, avatarPath, updatedBy) {
  ...
  const d = (v) => (!v || v.trim() === '') ? null : v.trim();
...
async function softDeleteEmployee(id, deletedBy) {
  try {
    const result = await pool.query(
      `UPDATE tbl_appusers SET is_deleted = true, updated_by = $1, updated_on = NOW() WHERE id = $2::bigint`,
      [deletedBy || null, id]
    );
    if (result.rowCount === 0) throw new Error(`No employee found with ID ${id}`);
    console.log(`softDeleteEmployee: Deleted employee ID ${id}`, result.rowCount, 'rows affected');
  } catch (err) {
    console.error(`softDeleteEmployee: Error deleting employee ID ${id}:`, err);
    throw err;
  }
}
...
module.exports = { findAllEmployees, findEmployeeById, createEmployee, updateEmployee, softDeleteEmployee, getSupervisors, findByEmail };
