    ...
    for (const employee of employees) {
      try {
        ...
        if (supervisorIds.length > 0 && typeof supervisorIds[0] === "string") {
          console.log(
            `⏭️  Employee ${employee.id}: Already has supervisor names, skipping`,
          );
          skipped++;
          continue;
        }
        ...
        if (validIds.length === 0) {
          console.log(
            `⚠️  Employee ${employee.id}: No valid supervisor IDs found`,
          );
          continue;
        }
        ...
        if (supervisors.length === 0) {
          console.log(
            `⚠️  Employee ${employee.id}: No supervisors found for IDs ${validIds.join(", ")}`,
          );
          continue;
        }
        ...
        console.log(
          `✅ Employee ${employee.id}: Converted ${validIds.join(", ")} → ${supervisorNames.join(", ")}`,
        );
        converted++;
      } catch (err) {
        console.error(
