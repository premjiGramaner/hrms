...
export default function DataTable<T>({
  ...
  const allColumns = hasActions
    ? [...columns, { key: "__actions__", header: "Actions", width: 160 } as ColumnDef<T>]
    : columns;

  const rowKey = (row: T, relIdx: number) =>
    getKey ? getKey(row, relIdx) : String((row as Record<string, unknown>).id ?? Math.random());
  ...
  const hoverRow = (e: React.MouseEvent<HTMLTableRowElement>, bg: string) => {
    Array.from((e.currentTarget as HTMLTableRowElement).cells).forEach(
      (td) => { (td as HTMLElement).style.background = bg; },
    );
  };
      ...
      {(onSearchChange || onAdd || extraToolbar) && (
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginLeft: "auto" }}>
            {searchQuery && (
              <span style={{
                fontSize: 12.5, color: "#64748b", background: "#f1f5f9",
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{
              background: "#1b2a6b", color: "#fff", borderRadius: 999,
              fontSize: 11, fontWeight: 700, padding: "1px 8px", marginLeft: 2,
            }}>{totalRecords}</span>
          </div>
        ...
        <div style={{
          ...
          <table style={{
            ...
            borderSpacing: "0 8px", fontSize: 13.5,
          }}>
            <thead>
                      ...
                      {sortable ? (
                        <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
                          ...
                          <span style={{ display: "inline-flex", flexDirection: "column", gap: 1, lineHeight: 1 }}>
                            <span style={{ fontSize: 8, color: sortable.dir === "asc" ? "#fff" : "rgba(255,255,255,0.4)", lineHeight: 1 }}>▲</span>
                            <span style={{ fontSize: 8, color: sortable.dir === "desc" ? "#fff" : "rgba(255,255,255,0.4)", lineHeight: 1 }}>▼</span>
                          </span>
                        </span>
                      ) : col.header}
                    </th>
                  ...
                  <td colSpan={allColumns.length + 1} style={{ ...tdFirst, ...tdLast, borderRight: "1px solid #e2e8f0", borderRadius: 10, textAlign: "center", padding: 56 }}>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
                      <div style={{
                        ...
                        border: "3px solid #e2e8f0", borderTopColor: "#1b2a6b",
                ...
                <tr>
                  <td colSpan={allColumns.length + 1} style={{ ...tdFirst, ...tdLast, borderRight: "1px solid #e2e8f0", borderRadius: 10, textAlign: "center", padding: 60 }}>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
                      ...
                      <span style={{ fontSize: 14, fontWeight: 600, color: "#475569" }}>{emptyTitle}</span>
                      <span style={{ fontSize: 13, color: "#94a3b8" }}>{emptySubtitle}</span>
                    </div>
                  </td>
                </tr>
              )}
                  ...
                  >
                    <td style={tdFirst}>
                      <span style={{
                        ...
                        width: 26, height: 26, background: "#f1f5f9",
                        borderRadius: 8, fontSize: 11.5, fontWeight: 700, color: "#64748b",
                      }}>
                        {absIdx + 1}
                      </span>
                    </td>
                    ...
                    {columns.map((col, cIdx) => {
                      ...
                      return (
                        <td key={col.key} style={isLastDataCol ? tdLast : tdBase}>
                          {col.render ? col.render(row, absIdx, relIdx) : String((row as Record<string, unknown>)[col.key] ?? "—")}
                        </td>
                      );
                    })}
                        ...
                        <div style={{ display: "flex", gap: 6 }}>
                          {actions.map((action) => (
                            <button
                              ...
                              onMouseEnter={(e) => {
                                if (action.bgHover) (e.currentTarget as HTMLButtonElement).style.background = action.bgHover;
                                if (action.borderColorHover) (e.currentTarget as HTMLButtonElement).style.borderColor = action.borderColorHover;
                              }}
                              onMouseLeave={(e) => {
                                (e.currentTarget as HTMLButtonElement).style.background = action.bg ?? "#f1f5f9";
                                (e.currentTarget as HTMLButtonElement).style.borderColor = action.borderColor ?? "#e2e8f0";
                              }}
                            >
                              {action.icon}
                              {action.label}
