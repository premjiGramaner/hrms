            ...
            {!loading &&
              roles.map((role, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      style={typeBadge(role.role_type)}
                      className="rounded-full px-2.5 py-0.5 text-xs font-semibold inline-block"
                    >
                      {role.role_type}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`text-xs font-semibold ${role.is_active ? "text-green-600" : "text-slate-400"}`}
                    >
                      {role.is_active ? "Enabled" : "Disabled"}
                    </span>
                  </td>
                  <td className="px-4 py-3 flex gap-2">
                    <button className="bg-none border-none cursor-pointer text-slate-400 text-base hover:text-slate-600 transition">
                      ✎
                    </button>
                    <button
                      onClick={() => {
                        if (confirm("Delete?"))
                          deleteRole(role.id).then(fetchRoles);
                      }}
    ...
    >
      <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden">
        <div className="px-6 pt-5 pb-0 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="w-7 h-7 rounded-full bg-slate-100 border-none cursor-pointer text-sm text-slate-600 hover:bg-slate-200 transition"
          >
            ✕
          </button>
        </div>
        <div className="px-6 py-4.5 flex flex-col gap-3.5">
          {error && (
            <div className="px-3 py-2 bg-red-50 text-red-900 rounded border-l-4 border-red-300 text-sm">
              {error}
            </div>
          )}
          <div>
            <label className="text-xs font-semibold text-slate-700 block mb-1.25">
