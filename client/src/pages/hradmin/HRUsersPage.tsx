    ...
    setFiltered(
      users.filter(
        (user) =>
          ...
          ((user.name || "") as string).toLowerCase().includes(searchTerm),
      ),
    );
  };
            ...
            {!loading &&
              filtered.map((user, i) => (
                ...
                >
                  ...
                  >
                    {user.role === "hradmin"
                      ? "Default ESS, Default Supervisor, Global Admin"
                      : user.role === "empmanager"
                        ? "Default ESS, Default Supervisor"
                        : "Default ESS"}
                  </td>
...
export function AddUserModal({
  ...
  const [form, setForm] = useState({
    employee_name: "",
    username: "",
    ess_role: "Default ESS",
    supervisor_role: "Default Supervisor",
    admin_role: "",
    status: "Enabled",
    set_password: true,
  });
        ...
        >
          <Row>
            ...
            <Field label="Username" required>
              {renderInput(form.username, (value) =>
                setForm((formState) => ({ ...formState, username: value })),
              )}
            </Field>
          </Row>
          <Row>
            <Field label="ESS Role" required>
              {renderSelect(
                form.ess_role,
                (value) =>
                  setForm((formState) => ({ ...formState, ess_role: value })),
                ["Default ESS", "Custom ESS"],
              )}
            </Field>
            <Field label="Supervisor Role" required>
              {renderSelect(
                form.supervisor_role,
                (value) =>
                  setForm((formState) => ({
                    ...formState,
                    supervisor_role: value,
                  })),
                ["Default Supervisor", "Custom Supervisor"],
              )}
            </Field>
          </Row>
          <Row>
            <Field label="Admin Role">
              {renderSelect(
                form.admin_role,
                (value) =>
                  setForm((formState) => ({ ...formState, admin_role: value })),
                ["", "Global Admin", "HR Admin"],
              )}
            </Field>
            <Field label="Status">
              <div
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 999,
                border: "none",
                background: "linear-gradient(90deg,#1b2a6b,#16a085)",
                color: "#fff",
                fontSize: 13.5,
                fontWeight: 700,
                cursor: "pointer",
