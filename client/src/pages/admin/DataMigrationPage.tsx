...
export default function DataMigrationPage() {
  ...
  const [initialStatus, setInitialStatus] = useState<MigrationStatus | null>(
    null,
  );
  ...
  const { status, setStatus, pollError } = useMigrationProgress(
  ...
  const handleFile = async (selected: File) => {
    ...
    try {
      ...
      setFile(null);
      Toast.error(
        error?.response?.data?.message || "Excel upload or validation failed",
      );
    } finally {
      setUploading(false);
    }
  ...
  const confirmStart = async () => {
    ...
    try {
      ...
      Toast.error(
        error?.response?.data?.message || "Could not start migration",
      );
    } finally {
      setStarting(false);
    }
  ...
  const download = async (
    id: number,
    type: "all" | "errors" | "validation" | "skipped" | "failed",
    format: "xlsx" | "csv",
  ) => {
    try {
  ...
  const errorColumns: DataTableColumn<MigrationValidationError>[] = [
    ...
    { key: "column", header: "Column", sortable: true },
    {
      ...
      render: (item) => (
        <span
          ...
          title={String(item.invalidValue ?? "")}
        >
    ...
    },
    {
      ...
      header: "Reason",
      render: (item) => (
        <span className="block min-w-[220px]">{item.reason}</span>
      ),
    },
    {
      key: "severity",
      header: "Severity",
      sortable: true,
      render: (item) => <StatusPill status={item.severity} />,
    },
  ];
  ...
  const historyColumns: DataTableColumn<MigrationStatus>[] = [
    ...
    { key: "failed_records", header: "Failed", sortable: true },
    {
      ...
      sortable: true,
      render: (item) =>
        formatDuration(item.duration_seconds ?? item.execution_time_seconds),
    },
    {
      key: "status",
      header: "Status",
  ...
  return (
    <Layout title="Data Migration">
      <div className="mx-auto max-w-[1500px] space-y-5">
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
          <div>
            <nav className="mb-2 text-xs font-medium text-slate-400">
              Dashboard <span className="mx-1">›</span> Administration{" "}
              <span className="mx-1">›</span>{" "}
              <span className="text-teal-600">Data Migration</span>
            </nav>
            <h1 className="text-2xl font-black text-slate-900">
              Data Migration
            </h1>
            <p className="mt-1 text-sm text-slate-500">
              Import legacy HRMS employee and master data into the new HRMS
              database.
            </p>
          </div>
          {canStart ? (
          ...
          <div className="flex flex-col gap-3 rounded-xl border border-blue-200 bg-blue-50 px-4 py-4 text-blue-900 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="font-bold">
                Validation complete — records are not inserted yet
              </p>
              <p className="mt-1 text-xs text-blue-700">
                {status.valid_records} records are ready. Start database
                insertion to write them to PostgreSQL.
              </p>
            </div>
            <Button
        ...
        {validation ? (
          <section>
            <div className="mb-3 flex items-end justify-between">
              <div>
                <h2 className="font-bold text-slate-800">Validation Summary</h2>
                <p className="text-xs text-slate-500">
                  Checks include required fields, data types, lookups,
                  duplicates, and constraints.
                </p>
              </div>
            </div>
        ...
        {uploadResult?.sheets.length ? (
          <SheetPreviewSection sheets={uploadResult.sheets} />
        ) : null}
        ...
        {status && terminalStatuses.has(status.status) ? (
          <Card className="border border-slate-200 p-5">
            <div className="mb-4 flex items-center gap-2">
              <Gauge size={19} className="text-teal-600" />
              <div>
                <h2 className="font-bold text-slate-800">Migration Summary</h2>
                <p className="text-xs text-slate-500">
                  Final execution totals for this workbook.
                </p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 xl:grid-cols-8">
              {[
                ["Total Sheets", status.total_sheets],
                ["Total Rows", status.total_records],
                ["Inserted", status.inserted_records],
                ...
                >
                  <p
                    className="truncate text-lg font-black text-slate-800"
                    title={String(value)}
                  >
                    {value}
                  </p>
        ...
        {status ? (
          <Card className="overflow-hidden border border-slate-200">
            <div className="flex flex-col gap-3 border-b border-slate-100 p-5 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">
                  Search, sort, filter, and export row-level exceptions.
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                <select
                  ...
                  className="cf-input w-auto py-2"
                >
                ...
                <Button
                  ...
                  onClick={() => download(status.id, "errors", "xlsx")}
                >
                  Export Errors
                </Button>
              </div>
            ...
            <DataTable
              ...
              getRowId={(item) =>
                item.id || `${item.sheet}-${item.row}-${item.column}`
              }
              emptyMessage="No validation or processing errors"
            />
            <Pagination
        ...
        <Card className="overflow-hidden border border-slate-200">
          <div className="flex flex-col gap-3 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="font-bold text-slate-800">Migration History</h2>
              <p className="mt-1 text-xs text-slate-500">
                Previous uploads, outcomes, and downloadable audit reports.
              </p>
            </div>
            ...
            actions={(item) => (
              <div className="flex min-w-[210px] gap-1.5">
                ...
                <button
                  ...
                  onClick={() => download(item.id, "all", "xlsx")}
                ...
                <button
                  ...
                  onClick={() => download(item.id, "all", "csv")}
                  className="rounded-lg bg-slate-100 px-2 text-[11px] font-bold text-slate-700 hover:bg-slate-200"
                >
                  LOG
                </button>
              </div>
        ...
        >
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-xl bg-slate-50 p-4">
                <p className="text-xs text-slate-500">Valid for Processing</p>
            ...
            {uploadResult?.tablesAffected.length ? (
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {uploadResult.tablesAffected.map((table) => (
                    <code
                      key={table}
                      className="rounded-lg bg-blue-50 px-2.5 py-1.5 text-xs text-blue-800"
                    >
                      {table}
                    </code>
                  ))}
            ...
            <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4">
              ...
              <span>
                ...
                <span className="text-xs leading-5 text-amber-700">
                  When enabled, existing records with the same unique key are
                  updated. Otherwise they are safely skipped.
                </span>
              </span>
            </label>
            {status.invalid_records ? (
              <p className="flex gap-2 rounded-xl bg-rose-50 p-3 text-xs text-rose-700">
                <AlertTriangle size={16} className="shrink-0" />
                {status.invalid_records} invalid records will be skipped and
        ...
        >
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-slate-500">
              All valid records were processed. Download the report for
              row-level details.
            </p>
            <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              <div className="rounded-xl bg-blue-50 p-3">
                <p className="flex items-center justify-center gap-1 text-xl font-black text-blue-700">
                  <Timer size={18} />
                  {formatDuration(status.execution_time_seconds)}
                </p>
