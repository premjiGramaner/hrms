...
function EmployeeAutocomplete({
  ...
  const [query, setQuery] = useState(
    value ? `${value.employee_id ? value.employee_id + " - " : ""}${value.name}` : ""
  );
        ...
        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-xl z-50 max-h-52 overflow-y-auto">
          {options.map((emp) => (
            ...
            >
              ...
              {emp.job_title && (
                <span className="text-xs text-slate-400 ml-2 flex-shrink-0">· {emp.job_title}</span>
              )}
            </button>
          ))}
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-4">
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1.5">
                Employee <span className="text-slate-400 font-normal">(optional)</span>
              </label>
              ...
              {!loading && !searched && (
                <tr>
                  <td colSpan={8} className="text-center py-16 text-slate-400 text-sm">
                    Click <span className="font-semibold text-slate-600">Search</span> to load entitlement records.
                  </td>
                </tr>
              )}
                ...
                <tr>
                  <td colSpan={8} className="text-center py-16 text-slate-400">
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      Try different filters or check that entitlements have been added.
                    </p>
                  </td>
                </tr>
              )}
