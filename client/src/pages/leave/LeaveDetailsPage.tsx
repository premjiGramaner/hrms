...
export default function LeaveDetailsPage() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      addToast(getApiErrorMessage(e, "Upload failed."), "error");
    } finally {
      setUploading(false);
  ...
  return (
    <LeaveLayout>
      ...
      {showReject && (
        <RejectModal onConfirm={handleRejectConfirm} onClose={() => setShowReject(false)} />
      )}
      {showCancel && (
        <CancelModal onConfirm={handleCancelConfirm} onClose={() => setShowCancel(false)} />
      )}
        ...
        <>
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5 mb-4">
            <div className="flex items-start gap-4">
              <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-900 to-teal-600 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                {leave.avatar ? (
                  <img
                    src={`/${leave.avatar}`}
              ...
              <div className="flex-1 min-w-0">
                ...
                {leave.job_title && (
                  <p className="text-xs text-slate-500 mb-2">{leave.job_title}</p>
                )}
                <div className="flex flex-wrap gap-x-6 gap-y-1 text-xs text-slate-600">
                  <span>
                    <span className="font-medium text-slate-500">Requested for: </span>
                    {leave.start_date}
                    {leave.start_date !== leave.end_date && ` to ${leave.end_date}`}
                  </span>
                  <span>
                    <span className="font-medium text-slate-500">Applied on: </span>
                ...
                <thead>
                  <tr className="bg-slate-50 border-b-2 border-slate-100">
                    {["Date", "Leave Type", "Net Leave Balance", "Duration (Days)", "Status", "Comments", "Actions"].map((h) => (
                      <th key={h} className="px-4 py-3 text-left text-xs font-bold text-slate-600 whitespace-nowrap">
                        {h}
                      </th>
                    ))}
                  </tr>
            ...
            {leave.attachment_path ? (
              <div className="flex items-center gap-3 mb-4 p-3 bg-slate-50 rounded-lg border border-slate-200">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <span className={`text-xs mt-0.5 inline-block px-2 py-0.5 rounded-full font-medium
