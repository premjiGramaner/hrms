...
function RejectModal({ leaveId, onConfirm, onCancel }: { leaveId: number; onConfirm: (r: string) => void; onCancel: () => void }) {
  const [reason, setReason] = useState("");
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
        <h3 className="text-base font-bold text-slate-800 mb-4">Reject Leave #{leaveId}</h3>
        <label className="block text-sm text-slate-600 mb-1">Rejection Reason <span className="text-red-500">*</span></label>
        <textarea value={reason} onChange={e => setReason(e.target.value)} rows={4} placeholder="Enter reason…"
        ...
        <div className="flex justify-end gap-2 mt-4">
          <button onClick={onCancel} className="px-4 py-2 text-sm rounded-lg border border-slate-300 text-slate-600 hover:bg-slate-50 cursor-pointer transition">Cancel</button>
          <button disabled={!reason.trim()} onClick={() => reason.trim() && onConfirm(reason.trim())}
            className="px-4 py-2 text-sm rounded-lg bg-red-600 text-white hover:bg-red-700 cursor-pointer transition disabled:opacity-50">Reject</button>
        </div>
      </div>
    </div>
  );
...
function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={`inline-block text-xs font-semibold px-2 py-0.5 rounded-full border ${map[status] || "bg-slate-50 text-slate-500 border-slate-200"}`}>{status}</span>;
}

function EmployeeAutocomplete({ value, onChange, onSelect }: {
  value: string;
  ...
  return (
    <div ref={containerRef} className="relative">
      <input type="text" value={value} onChange={e => onChange(e.target.value)} placeholder="Type name, ID or username…"
      ...
      {open && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-xl z-50 max-h-52 overflow-y-auto">
          {suggestions.map(emp => (
            <button key={emp.id} type="button" onClick={() => { onSelect(emp); setOpen(false); onChange(emp.name); }}
              className="w-full text-left px-3 py-2 text-sm hover:bg-slate-50 cursor-pointer transition flex items-center gap-2">
              <span className="font-mono text-xs text-slate-400 w-20 flex-shrink-0">{emp.employee_id || emp.username}</span>
...
export default function LeaveListPage() {
  ...
  const [filterOpts, setFilterOpts] = useState<FilterOptions>({ sub_units: [], locations: [], job_titles: [], employment_statuses: [], job_categories: [] });
  ...
  const toggleStatus = (status: string) => {
    setForm(prev => {
      ...
      if (status === "All") return { ...prev, statuses: s.length === STATUS_OPTIONS.length ? [] : [...STATUS_OPTIONS] };
      return { ...prev, statuses: s.includes(status) ? s.filter(x => x !== status) : [...s, status] };
    });
  };
  ...
  const handleApprove = useCallback(async (id: number) => {
    setActionLoading(id);
    try { await approveLeave(id); addToast("Leave approved.", "success"); dispatch(fetchLeaves({ ...filters })); }
    catch (e) { addToast(getApiErrorMessage(e, "Failed to approve."), "error"); }
    finally { setActionLoading(null); }
  }, [filters]);

  const handleRejectConfirm = useCallback(async (reason: string) => {
    if (!rejectTarget) return;
    setActionLoading(rejectTarget); setRejectTarget(null);
    try { await rejectLeave(rejectTarget, reason); addToast("Leave rejected.", "success"); dispatch(fetchLeaves({ ...filters })); }
    catch (e) { addToast(getApiErrorMessage(e, "Failed to reject."), "error"); }
    finally { setActionLoading(null); }
  ...
  const handleCancel = useCallback(async (id: number) => {
    ...
    setActionLoading(id);
    try { await cancelLeave(id); addToast("Leave cancelled.", "success"); dispatch(fetchLeaves({ ...filters })); }
    catch (e) { addToast(getApiErrorMessage(e, "Failed to cancel."), "error"); }
    finally { setActionLoading(null); }
  }, [filters]);

  const handleExport = async (type: "summary" | "detail") => {
    try { type === "summary" ? await exportSummaryExcel(filters) : await exportDetailExcel(filters); }
    catch (e) { addToast(getApiErrorMessage(e, "Export failed."), "error"); }
  };

  const inputCls = "w-full border border-slate-300 rounded px-2.5 py-1.5 text-sm outline-none focus:border-blue-400 bg-white transition";
  const selectCls = "w-full border border-slate-300 rounded px-2.5 py-1.5 text-sm outline-none focus:border-blue-400 bg-white transition appearance-none cursor-pointer";
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-slate-50 border-b-2 border-slate-100">
                {["Employee ID", "Employee Name", "Date", "Applied On", "Leave Type", "Net Leave Balance", "Requested Duration", "Status", "Actions"].map(h => (
                  <th key={h} className="px-3 py-2.5 text-left text-xs font-bold text-slate-600 whitespace-nowrap">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
        ...
        {data && data.totalPages > 1 && (
          <div className="px-4 py-3 border-t border-slate-100 flex items-center gap-3 text-sm text-slate-600">
            <button onClick={() => handlePageChange((filters.page || 1) - 1)} disabled={(filters.page || 1) <= 1}
              className="px-4 py-1.5 rounded border border-slate-200 bg-white cursor-pointer text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-50 transition">← Prev</button>
            <span className="text-xs text-slate-500">Page {data.page} of {data.totalPages} · {data.total} records</span>
            <button onClick={() => handlePageChange((filters.page || 1) + 1)} disabled={(filters.page || 1) >= data.totalPages}
              className="px-4 py-1.5 rounded border border-slate-200 bg-white cursor-pointer text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-50 transition">Next →</button>
          </div>
        )}
      </div>
    </LeaveLayout>
