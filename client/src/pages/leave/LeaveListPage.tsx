...
import { fetchLeaves, setFilters, resetFilters } from "../../store/leaveSlice";
import {
  getLeaveTypes,
  approveLeave,
  rejectLeave,
...
const SUB_UNITS = ["Networking", "IT Service", "Finance", "Admin", "HR", "Delivery", "Sales"];
const LOCATIONS = ["Bangalore", "Coimbatore", "Hyderabad"];
const JOB_TITLES = ["HR", "Developers"];
const EMP_STATUSES = ["Full Time"];
...
function RejectModal({ leaveId, onConfirm, onCancel }: RejectModalProps) {
...
export default function LeaveListPage() {
  ...
  useEffect(() => {
    const initialFilters = { ...form, page: 1 };
    dispatch(setFilters(initialFilters));
    dispatch(fetchLeaves(initialFilters));
  ...
  const toggleStatus = (status: string) => {
    setForm((prev) => {
      ...
      if (status === "All") {
        return { ...prev, statuses: statuses.length === STATUS_OPTIONS.length ? [] : [...STATUS_OPTIONS] };
      }
      return {
        ...prev,
...
function ActionDropdown({ row, isManager, loading, onApprove, onReject, onCancel }: ActionDropdownProps) {
