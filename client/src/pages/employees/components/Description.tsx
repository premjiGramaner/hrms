...
export function DescriptionCell({ description }: DescriptionCellProps) {
  ...
  const descriptionPreview  = description.length > DESCRIPTION_PREVIEW_LENGTH ? `${description.slice(0,DESCRIPTION_PREVIEW_LENGTH)}...` : description
  return (
    <div className="relative inline-block group">
        <span title={description} className="text-slate-600 text-[13px] cursor-pointer">
        {descriptionPreview}
        </span>
